...
2. **CPU Considerations:**
   - Let's assume each vCPU can handle approximately 100-150 threads (or coroutines) effectively, actually it could handle much more depending on workload. But we can safely assume that number of threads as a safe margin for this setup, based on [Kotlin async coroutines benchmark](https://github.com/mmoraesbr/kotlin-coroutine-benchmark).
